...
{-|
Module      : Conso.Fr.Elec.Sge.DemandePublicationMesuresFacturantesM23V10Type
Description : Types Haskell générés depuis le WSDL CommandeHistoriqueDonneesMesuresFacturantes v1.0
...
elementDemandePublicationMesuresFacturantes :: XMLParser DemandePublicationMesuresFacturantes
elementDemandePublicationMesuresFacturantes = parseSchemaType "demandePublicationMesuresFacturantes"
elementToXMLDemandePublicationMesuresFacturantes :: DemandePublicationMesuresFacturantes -> [Content ()]
elementToXMLDemandePublicationMesuresFacturantes = schemaTypeToXML "sc:demandePublicationMesuresFacturantes"
...
data DemandePublicationMesuresFacturantes = DemandePublicationMesuresFacturantes
        ...
        deriving (Eq,Show)
instance SchemaType DemandePublicationMesuresFacturantes where
    parseSchemaType s = do
        (_,e) <- posnElement [s]
        commit $ interior e $ return DemandePublicationMesuresFacturantes
            `apply` parseSchemaType "donneesGenerales"
            `apply` parseSchemaType "demande"
    schemaTypeToXML s x@DemandePublicationMesuresFacturantes{} =
        toXMLElement s [ toXMLAttribute "xmlns:sc" $ Xsd.XsdString "https://sge-b2b.enedis.fr/services/commandehistoriquedonneesmesuresfacturantes/v1" 
                       ]
            [ schemaTypeToXML "donneesGenerales" $ demandePublicationMesuresFacturantes_donneesGenerales x
            , schemaTypeToXML "demande" $ demandePublicationMesuresFacturantes_demande x
            ]
