...
{-|
Module      : Conso.Fr.Site.SiteDB.Elec.Ingestion.GapFill
Description : Combler les lacunes élec via les demandes M023

Ce module orchestre le processus en deux étapes :

  1. __Détecter__ les plages manquantes dans la base site (via "Conso.Fr.Site.SiteDB.Elec.Storage.Gaps")
...
('GapFillCallbacks'). Cela évite une dépendance circulaire entre @conso-site-db@
...
import           Conso.Fr.Site.Registry                            (openRegistry, lookupByPrm)
import           Conso.Fr.Site.SiteDB.Elec.Types.Common            (PrmId(..), Periode(..), Pas(..))
import           Conso.Fr.Site.SiteDB.Elec.Storage.Connection      (openSiteDb)
import           Conso.Fr.Site.SiteDB.Elec.Storage.Gaps
import           Conso.Fr.Site.SiteDB.Elec.Ingestion.FromRfiles    (ingestDirectory, IngestDirResult)
