...
{-|
Module      : Conso.Fr.Site.Registry
Description : Façade pour l'accès au registre central des sites

Fournit une fonction d'ouverture du registre (@registry.db@) et
ré-exporte les opérations CRUD depuis "Conso.Fr.Site.Registry.Operations".

Usage typique :

@
import Conso.Fr.Site.Registry
...
module Conso.Fr.Site.Registry
  ...
  , module Conso.Fr.Site.Registry.Operations
  ) where

import           Database.SQLite.Simple
import           System.FilePath                    ((</>))
import           System.Directory                   (createDirectoryIfMissing)
import           Conso.Fr.Site.Registry.Schema      (ensureRegistrySchema)
import           Conso.Fr.Site.Registry.Operations
