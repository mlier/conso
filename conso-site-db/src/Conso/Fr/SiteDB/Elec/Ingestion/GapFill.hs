...
{-|
Module      : Conso.Fr.SiteDB.Elec.Ingestion.GapFill
Description : Combler les lacunes élec via les demandes M023

Ce module orchestre le processus en deux étapes :

  1. __Détecter__ les plages manquantes dans la base site (via "Conso.Fr.SiteDB.Elec.Storage.Gaps")
...
import           Conso.Fr.SiteDB.Elec.Types.Common            (PrmId(..), Periode(..), Pas(..))
import           Conso.Fr.SiteDB.Elec.Storage.Connection      (openSiteDb)
import           Conso.Fr.SiteDB.Elec.Storage.Gaps
import           Conso.Fr.SiteDB.Elec.Ingestion.FromRfiles    (ingestDirectory, IngestDirResult)
